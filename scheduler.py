...
def check_lesson_completion():
    ...
    global _bot
    print("🔍 check_lesson_completion вызвана")
    if not _bot:
        print("❌ _bot = None в check_lesson_completion")
        return
    ...
    try:
        ...
        with next(get_db()) as db:
            ...
            print(f"🔍 check_lesson_completion: найдено занятий для проверки: {len(bookings)}")
            ...
            for booking in bookings:
                try:
                    ...
                    print(f"🔍 Занятие {booking.id}: lesson_datetime={lesson_datetime}, end_datetime={end_datetime}, time_to_end={time_to_end}")
                    ...
                    if timedelta(minutes=10) <= time_to_end <= timedelta(minutes=15):
                        ...
                        if user and user.user_id:
                            ...
                            print(f"❌ Пользователь не найден для занятия {booking.id}")
...
def run_scheduler():
    ...
    print("✅ check_lesson_completion запланирована каждые 5 минут")
